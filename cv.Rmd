...
```{r save-positions, include=FALSE}
process_projects <- function(position_data) {
  position_data %>%
    # Filter
    dplyr::filter(
    ...
    dplyr::select(
      "title", "link", "short_summary", "description_bullets"
    ) %>% 
    
    # Create transparent link_text from link
    dplyr::mutate(
      link_text = stringr::str_remove(.data$link, "^https?://"),
      link_text = stringr::str_replace(link_text, "^(([^/]+/[^/]+)).*$", "\\1"),
      link_text = stringr::str_remove(link_text, "/$")
    ) %>%   
    
    # Extract tools from processed description
    dplyr::mutate(tools = stringr::str_match(
      .data$description_bullets, "\\(([^)]*)\\)$")[, 2]
    ) %>%
    dplyr::select(-.data$description_bullets)
}

utils::write.csv(
  process_projects(position_data),
  file.path(get_path_to("input"), "projects.csv"),
  row.names = FALSE
)
