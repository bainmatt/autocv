#' @keywords internal
"_PACKAGE"

# The following block is used by usethis to automatically manage
# roxygen namespace tags. Modify with care!
## usethis namespace: start
#' @importFrom autocv autocv_resources get_path_to
#' @importFrom here here
#' @importFrom graphics text
#' @importFrom magrittr "%>%"
#' @importFrom rvest read_html
#' @importFrom usethis use_data
#' @importFrom withr with_envvar
#' @importFrom readxl read_excel
#' @importFrom writexl write_xlsx
#' @importFrom purrr map_chr reduce
#' @importFrom tidyselect ends_with
#' @importFrom pagedown chrome_print
#' @importFrom tibble add_row tibble
#' @importFrom stats na.omit reorder
#' @importFrom assertthat assert_that
#' @importFrom glue glue glue_collapse
#' @importFrom yaml read_yaml write_yaml
#' @importFrom rlang has_name sym ":=" .data
#' @importFrom utils read.csv write.csv View
#' @importFrom fs is_dir is_file path_dir path_rel
#' @importFrom stringr fixed str_c str_detect str_match_all
#' @importFrom cli cli_alert_danger cli_alert_success cli_alert_warning
#' @importFrom ggplot2
#'  aes coord_flip expand_limits
#'  element_rect geom_col geom_text labs theme theme_void
#' @importFrom dplyr
#'  across arrange case_when c_across desc everything filter first group_by
#'  if_else last matches mutate pull rowwise row_number select starts_with
#'  ungroup where
## usethis namespace: end
NULL
