...
construct_app_metadata <- function(
  ...
  # data_filepath    <- file.path(find_project_root(), data_relpath)
  ...
  app_df <- data.frame(
    ...
    # base_resume_data = NA_character_,
    # base_cover_data  = NA_character_,
    ...
    # TODO ?Add extension within routines for opening files
...
#' 
#' `get_path_to_app` returns an abs path to a document from a given application.
#' 
#' `open_app` opens the requested document for a given application.
#'
#' @family cli
#' @export
get_app_info <- function(
  ...
  log <- load_log(app_dir = app_dir, app_period = app_period)
  ...
  if (!all(field == "all")) {
    app_df <- app_df %>% 
      # TODO explicitly state that this function is vectorized over fields/ids
...
open_app <- function(
  ...
  if (stringr::str_ends(document, "url")) {
    url <- get_app_info(
      id = id,
      field = document,
      company = company,
